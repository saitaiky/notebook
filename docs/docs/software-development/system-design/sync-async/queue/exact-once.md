...
![The Two Generals Problem](/img/software-development/system-design/sync-async/queue/exactly-once-image-9.png)
...
![Msg being altered](/img/software-development/system-design/sync-async/queue/exactly-once-image-10.png)
Source: [singlestore - Getting to Exactly-Once Semantics with Apache Kafka
and SingleStore Pipelines (Webcast On-Demand)](https://www.singlestore.com/blog/exactly-once-semantics-with-apache-kafka/)
...
On the [at-least-once](/software-development/system-design/sync-async/queue/at-least-once) page, there are a lot of reliability problems with this push approach. The most basic problem is that sometimes messages are delivered, and sometimes messages are not delivered. Also, there is the uncertainty of not knowing what happened on the other side of the wire.

![at-least-once](/img/software-development/system-design/sync-async/queue/atotm-at-least-once-06.png)
...
Ok, so the push messaging approach is solvable but somewhat complex when it comes to reliable messaging. What about the pull approach? The pull approach is slightly counter-intuitive, but it is typically less complicated to implement. Both the push and pull approaches were covered in detail in [at-least-once](/software-development/system-design/sync-async/queue/at-least-once) of this series so please refer to that document for more details.

The push and pull approaches provide ways for implementing at-least-once delivery while the commonly used synchronous HTTP REST approach without retry offers at-most-once delivery, as discussed in [at-most-once](/software-development/system-design/sync-async/queue/at-most-once).
...
![Example messaging setup](/img/software-development/system-design/sync-async/queue/exactly-once-image-12.png)

The message bus also provides transactional guarantees. Once a given message is successfully delivered to the message bus it guarantees that message is eventually delivered to the message receivers or consumers. One of the challenges in this message delivery flow is the non-transactional gaps between the event bus and the message senders and receivers, as shown in above image. The details for handling this were also covered in [at-most-once](/software-development/system-design/sync-async/queue/at-most-once).

![Transactionally store offset and state together](/img/software-development/system-design/sync-async/queue/exactly-once-image-13.png)
