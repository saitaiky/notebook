...
For example, the customer might ask: “What are the ML algorithms that Databricks supports out-of-the-box?” While an SA might know all of the algorithms, it is important to ask the customer if he/she has any preference for a particular algorithm and what he/she is currently using. This ensures that when an SA comes back with an answer, the answer can be tailored to the prospect’s specific requirement.
