...
> TL;DR
   - The difference between them is that AWS WAF provides protection on the **application layer (layer 7)** and AWS Shield protects the **infrastructure layers (layer 3/4)** of the OSI model.
   - AWS WAF is for common web exploits such as cross-site scripting, SQL injection, and brute-force HTTP flood attacks.
