...
:::infoWhat is ACM?
AWS ACM (Amazon Certificate Manager) is a service that provides free, automated SSL/TLS certificates for secure encryption and authentication of data in transit for AWS resources and applications.

Important: The ACM certificate must be requested or imported **in the same AWS Region as your load balancer or CloudFront distribution.**
:::
...
There is only one algorithm which only work for **NLB**, which is Flow Hash
- Selects a target based on the protocol, source/destination IP address, source/destination por t, and TCP sequence number
...
For a detailed explaination, visit [Ephemeral port : What is it and what does it do?](/linux/network/ip-dns/#ephemeraldynamictemporary-port)
...
> TL;DR - 4XX codes are client-side errors and 5xx errors are going to be server problems
