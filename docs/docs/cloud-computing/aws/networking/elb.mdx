...
Requests originating from Elastic Load Balancing use ports 1024-65535. For a detailed explaination, visit [Ephemeral port : What is it and what does it do?](/linux/network/ip-dns/#ephemeraldynamictemporary-port)

### Target group contains only unhealthy targets?

If a target group contains only unhealthy targets, ELB routes requests across its unhealthy targets. It assumes that the health check is wrong, but maybe the health, the instance can still work for some cases, okay. This is a best effort scenario. 
