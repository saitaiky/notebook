...
> TL;DR - Enables **private connectivity** to services hosted in AWS, from within your VPC withoutusing and Internet Gateway, VPN, NAT devices, or firewall proxies.
...
A BGP connection session is established between AWS Direct Connect and your router on the public VIF. Another BGP session or a static route will be established between the virtual private gateway and your router on the IPsec VPN tunnels.
...
The route table in the VPC and ensure that there is a route that sends traffic to the on-premises network via the virtual private gateway. This is achieved by adding a Virtual Private Gateway (VGW) route with the destination of your on-premises network in the route table.
...
- Network Access Control List
    ...
    - Rules are evaluated starting with the **lowest numbered rule**. As soon as a rule matches traffic, it's applied regardless of any higher-numbered rule that might contradict it.Asterisk is for 包底
...
:::infoVPC flow log to show ACL is stateless
