...
**Enabling logging is free, but log files stored in the bucket incur storage charges**. You can delete log files as needed, and subsequent access to log files incurs standard charges. Logging is disabled by default, and logs are stored in the same AWS Region as the source bucket.
