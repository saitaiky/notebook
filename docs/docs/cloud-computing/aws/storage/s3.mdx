...
There are 2 approaches to improve upload and download speed across different regions. They're S3 Transfer Acceleration and Amazon CloudFront’s PUT/POST. According to [AWS- General S3 FAQs](https://aws.amazon.com/s3/faqs/), we should choose..
...
> **Amazon S3 transfer acceleration is using Cloudfront under the scene.** As the data arrives at an edge location, data is routed to Amazon S3 over an optimized network path.
