...
### Sepcify the right resources to actions
...
If you don't have either `arn:aws:s3:::tutorialsdojo/*` for `s3:GetObject` or `arn:aws:s3:::tutorialsdojo` for `s3:ListBucket`, you will be prompted with an "Action does not apply to any resource(s) in statement" error if you tried to apply this bucket policy. 
...
- Glacier Vault provides **[S3 Glacier Vault Lock](https://docs.aws.amazon.com/amazonglacier/latest/dev/vault-lock.html) to prevent future changes for WORM compliance**.
