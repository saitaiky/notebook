...
A retention period safeguards an object version for a specified duration in Amazon S3. When applied, a timestamp is added to the object version's metadata to mark the expiration of the retention period. Once the period ends, the object version can be modified or deleted, unless a legal hold is also in place. This is where the lifecycle policy kicks in and deletes the object 5 years after the object is created.

You can set a retention period explicitly by defining a "Retain Until Date" for the object version, or use a bucket default setting. Amazon S3 preserves the object version until the specified retention period concludes based on the "Retain Until Date" metadata.
...
1. Create a new CF distribution
  ...
  - Configure origin S3 bucket access to use a CloudFront origin access identity (OAI). While you can automatically update the bucket policy (with `s3:GetObject` access) as a starting point, we'll be adding/changing that to `s3:PutObject` anyway.
...
S3 Cross Account refers to a configuration in Amazon Simple Storage Service (S3) that allows access and sharing of S3 buckets across multiple AWS accounts. It enables a secure and controlled way to share data between different AWS accounts while maintaining granular access control.

With S3 Cross Account, you can grant permissions to users, groups, or roles from other AWS accounts to access and interact with your S3 buckets. This feature is particularly useful in scenarios where you have separate AWS accounts for different teams or organizations and need to collaborate or share data between them.
...
> 
  ...
  - Glacier Vault provides [S3 Glacier Vault Lock](https://docs.aws.amazon.com/amazonglacier/latest/dev/vault-lock.html) for WORM compliance.
