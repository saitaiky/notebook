...
![event-driven-architecture-example](/img/web-development/system-design/event-driven-architecture-example.png)
...
There are two main types of routers used in event-driven architectures: **event buses** and **event topics**. At AWS, we offer [Amazon EventBridge](/eventbridge/) to build event buses and [Amazon Simple Notification Service (SNS)](/sns/) to build event topics.
