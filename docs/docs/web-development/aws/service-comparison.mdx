...
![roles-usingrole-ec2roleinstance](/img/web-development/aws/security/roles-usingrole-ec2roleinstance.png "image_tooltip")

So this is a revolution, because CloudFormation template is YAML, is not type-safe, you can make errors and you only know at the very end if it worked or not. But with the CDK, you can define those using constructs and a programming language.
...
* Kinesis Data Streams provides support for real-time processing of streaming data. It provides ordering of records, as well as the ability to read and/or replay records in the same order to multiple downstream Amazon Kinesis Applications. [Output係data processing tool (Consumer)]
* Kinesis Data Firehose is used to load streaming data into data stores ( S3, Redshift, Elasticsearch Service, and Splunk)  [Output 係datastore]
...
Stream: You need to manually provision shards in case the load increases or you need to use CloudWatch alarms to set up auto scaling for the shards. 
...
(都明明地Kinese Producer Library 做咩)
...
[https://liveroomlk.medium.com/cloudfront-signed-urls-cookies-and-s3-presigned-urls-be850c34f9ce](https://liveroomlk.medium.com/cloudfront-signed-urls-cookies-and-s3-presigned-urls-be850c34f9ce)


### EventBridge vs Step functions for tracking workflow 

其實都唔係真正既一個比較, 但寫係到先。When you are in exam, and the case that they give is a step by step logic flow instead of event-driven case - then you can use Step function 
