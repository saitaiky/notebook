...
title: ECS & ECR
sidebar_position: 3
description: "Container services: ECR for storing Docker images with registry management, ECS for deploying containerized applications with tasks, services, and cluster orchestration."
keywords:
  - aws
  - amazon web services
  - compute
...
By default, ECR repository tags are **mutable** — pushing a new image with the same tag (e.g., `latest`) silently overwrites what that tag points to, which makes it hard to guarantee that the image running in production is exactly the one you tested. Enabling **tag immutability** on a repository prevents any existing tag from being overwritten; a push that reuses an existing tag is rejected, forcing you to push a new, uniquely tagged image instead.

:::tip Exam trap: reproducible deployments and rollback traceability
If a scenario emphasizes needing to **guarantee the exact image referenced by a deployment can never silently change** (a common audit/compliance and rollback-safety requirement), the answer is enabling **image tag immutability** on the ECR repository — not a bucket policy or IAM restriction, since those don't stop legitimate ECR push permissions from overwriting a tag.
:::
...
:::info When to set desired count of a task more than 1?
...
:::info Why EC2 Instances Remain the Same When Increasing Desired Tasks
...
Containers depend on Linux [processes](/linux/process/) plus [namespaces and cgroups](/linux/namespace/) for isolation and
resource control. ECS adds task placement, replacement, networking, identity, and service lifecycle on top of those
mechanisms; ECR stores the images. Use the [system-design hub](/software-development/system-design/) to decide service
boundaries before selecting a container scheduler, and compare [EKS](/aws/development/eks/) when Kubernetes APIs or its
ecosystem are a requirement.
