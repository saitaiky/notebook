...
title: EC2
sidebar_position: 1
description: Elastic Compute Cloud instances providing resizable compute capacity, including spot instances, reserved instances, and spot fleet allocation strategies for cost optimization.
keywords:
  - aws
  - amazon web services
  - compute
...
![instance-Type-Naming](/img/aws/compute/ec2/instance-Type-Naming.png)

![AWS-Instance-Types](/img/aws/compute/ec2/AWS-Instance-Types.png)
...
![ec2-allocation](/img/aws/compute/ec2/ec2-allocation.png)
...
<details>
  ...
  <div>
    ...
    <br/>
    <div>
    To create an inexpensive and diversified fleet, use the <em>lowestPrice</em> strategy in combination with <em>InstancePoolsToUseCount</em>. For example, if your target capacity is 10 Spot Instances, and you specify 2 Spot capacity pools (for <em>InstancePoolsToUseCount</em>), Spot Fleet will draw on the two lowest-priced pools to fulfill your Spot capacity.
    </div>
    <div>
    You can use a low or high number of Spot capacity pools across which to allocate your Spot Instances. For example, if you run batch processing, it is recommended to specify a low number of Spot capacity pools (for example, <em>InstancePoolsToUseCount</em>=2) to ensure that your queue always has compute capacity while maximizing savings.
    </div>
    <div>
    If you run a web service, AWS recommends specifying a high number of Spot capacity pools (for example, <em>InstancePoolsToUseCount</em>=10) to minimize the impact if a Spot capacity pool becomes temporarily unavailable.
    </div>
    <br/>
    <div>
...
:::info Upgrade and downgrade instance type
...
> To optimize EBS performance for handling a high volume of transactions from your EC2 instance, you can change your instance type to those EBS–optimized instances. [Supported instances](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ebs-optimized.html#ebs-optimization-support)

An Amazon EBS–optimized instance uses an optimized configuration stack and provides additional, dedicated capacity for Amazon EBS I/O. This optimization provides the best performance for your EBS volumes by minimizing contention between Amazon EBS I/O and other traffic from your instance.

### T2 Unlimited Mode

![t2-t3-bucket](/img/aws/compute/ec2/t2-t3-bucket.png)
...
:::info Make sure instance profile has enough permission to access other AWS services
...
Source: [Full comparsion table](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/dedicated-hosts-overview.html#dedicated-hosts-dedicated-instances)
...
> Reference: [Linux AMI virtualization types (HVM & PV)](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/virtualization_types.html)
...
:::warning AWS Backup does not reboot EC2 instances at any time.
...
Screenshot to show you can't set the reboot parameter on AWS Backup:
![amazon_aws_backup_ebs_noreboot_only](/img/aws/compute/ec2/amazon_aws_backup_ebs_noreboot_only.png)
:::

:::info AWS Backup with tags
...
In addition, AWS Backup leverages AWS Organizations to implement and maintain a central view of backup policy across resources in a **multi-account AWS environment**.
...
![copy-AMI](/img/aws/compute/ec2/copy-AMI.png)
...
:::info Change EC2 instance type in an Auto Scaling group 
...
EC2 exposes a virtual machine, so operating-system behaviour remains part of the architecture. Review the
[Linux learning path](/linux/), [boot sequence](/linux/loading-sequence/), [process lifecycle](/linux/process/), and
[filesystems](/linux/file-system/) when diagnosing instance behaviour. For secure administration, compare
[SSH](/linux/security/ssh/) with [Systems Manager Session Manager](/aws/management/ssm/). For higher-level runtime choices,
continue with [ECS](/aws/compute/ecs-ecr/) or [Lambda](/aws/compute/lambda/).
