...
sidebar_position: 2
description: Auto Scaling group service managing EC2 instance scaling with dynamic and manual options, health checks, instance warm-up, and availability zone rebalancing.
keywords:
  - aws
  - amazon web services
  - compute
...
:::warning Launch Configurations are legacy — use Launch Templates instead
AWS no longer recommends **Launch Configurations** for new Auto Scaling groups. **Launch Templates** are the modern replacement and support features launch configurations don't, such as **versioning** (create new versions instead of a brand-new resource each time), mixing **On-Demand and Spot Instances** in the same ASG, specifying **multiple instance types**, and referencing the **latest** or a **specific numbered** template version. If a question mentions needing a mix of Spot/On-Demand capacity or template versioning within an ASG, that's a Launch Template feature — Launch Configurations can't do it.
:::

### ASG terminates unhealthy EC2 instance

:::info Scaling without ASG update the size of the group
...
:::warning Scaling based on Amazon SQS
...
- CMK and Auto Scaling group are in different AWS accounts.
  ...
  2. Allow the Auto Scaling group account access to the CMK. Define an IAM user or role in the Auto Scaling group account that can create a grant.
  3. Create a grant to the CMK with the service-linked role as the grantee principal. 
  4. Update the Auto Scaling group to use the service-linked role.
...
:::warning Don't use ALB and a target group with the instance(s) of the Auto Scaling Group
