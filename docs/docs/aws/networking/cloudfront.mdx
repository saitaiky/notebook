...
title: CloudFront
sidebar_position: 4
description: Content delivery network distributing content globally with origin failover, signed URLs, cookies, and origin shield for cache optimization.
keywords:
  - aws
  - amazon web services
  - networking
...
- [New Amazon CloudFront Reporting – Learn More About Your Viewers](https://aws.amazon.com/blogs/aws/cloudfront-viewer-reports/)
...
:::tip Signed URL vs Signed Cookie: pick by number of restricted files
Use a **signed URL** when you need to restrict access to **individual files** (e.g., one installer download), or when your client doesn't support cookies. Use **signed cookies** when you need to grant access to **multiple restricted files** at once (e.g., every asset in a video streaming session) without generating and distributing a signed URL per file.
:::

### Origin Access Control (OAC) replaces Origin Access Identity (OAI)

When CloudFront serves private content from an S3 bucket, CloudFront needs a way to authenticate to S3 so the bucket itself can stay fully private (no public access). AWS's original mechanism for this was **Origin Access Identity (OAI)**, a special CloudFront principal you referenced in the bucket policy. **Origin Access Control (OAC)** is the newer, recommended replacement:

- OAC supports **SSE-KMS encrypted S3 objects**; OAI does not.
- OAC works with **all AWS Regions**, all S3 API operations (including `PUT`, not just `GET`), and follows AWS Signature Version 4 for every request.
- AWS now recommends OAC for all new distributions; OAI is a legacy feature kept for backward compatibility.

:::warning Exam trap: OAI/OAC only work with the S3 REST endpoint, not the S3 static website endpoint
Both OAI and OAC authenticate CloudFront to S3 using the bucket's **REST API endpoint** (`bucket.s3.region.amazonaws.com`). If the origin is instead configured as an **S3 static website endpoint** (`bucket.s3-website-region.amazonaws.com`), the bucket **must be public** — neither OAI nor OAC can be used, and CloudFront can only reach it over HTTP. Use the REST endpoint as the origin whenever you need CloudFront to keep the bucket private.
:::
...
- If you want CloudFront to cache different versions of your objects based on the device that a user is using to view your content, use `User-Agent` header. You can create values like CloudFront-Is-Desktop-Viewer, CloudFront-Is-Mobile-Viewer, etc
  ![s3web-specific-cloudfront](/img/aws/networking/cloudfront/s3web-specific-cloudfront.jpg)
  Based on the value of the `User-Agent` header, CloudFront sets the value of these headers to true or false before forwarding the request to your origin. If a device falls into more than one category, more than one value might be true. 
...
Reference: [Specifying the signers that can create signed URLs and signed cookies](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/private-content-trusted-signers.html)
...
![cf_dist_edit_behavior](/img/aws/networking/cloudfront/cf_dist_edit_behavior.png)
...
![static-dynamic-content](/img/aws/networking/cloudfront/static-dynamic-content.png)
