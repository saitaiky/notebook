...
title: VPC
sidebar_position: 1
description: Virtual private cloud service providing isolated network with subnets, routing, security groups, and resource scope across regions and availability zones.
keywords:
  - aws
  - amazon web services
  - networking
...
- Virtual Private Cloud
  ...
  - Network ACLs -- Regional (associated per subnet)
...
Both filter traffic in and out of your VPC, but they operate at different scopes and with a fundamentally different behavior that's a frequent exam trap:

- **Security Groups are stateful and instance/ENI-level.** If you allow an inbound request, the response is **automatically allowed out**, regardless of outbound rules. Security Groups only support `Allow` rules; there's no explicit `Deny`.
- **Network ACLs (NACLs) are stateless and subnet-level.** Allowing inbound traffic does **not** automatically allow the matching outbound response — you must explicitly open the **ephemeral port range** (1024-65535, or the client's actual ephemeral range) outbound for return traffic to flow. NACLs evaluate rules **in numeric order** and support both `Allow` and `Deny` rules.

:::danger Exam trap: Forgetting ephemeral ports on a NACL
A classic scenario: a NACL allows inbound port 443 but a client still can't complete an HTTPS request. The missing piece is almost always the **outbound NACL rule for the ephemeral port range** the client used for its source port — without it, the stateless NACL never lets the response back out. Security Groups never have this problem because they're stateful.
:::
...
:::info What is piblic subnet and private subnet
...
:::info 2 reasons the Packet is truncated when sending to the mirror target
...
:::warning It is so easy to get messy
...
- **Service coverage**: Interface endpoints cover almost every other AWS service exposed via PrivateLink — for example Kinesis, Secrets Manager, ECR, CloudWatch Logs, SSM, SNS, and many more.
...
:::danger Exam trap: Gateway endpoints only exist for S3 and DynamoDB
Gateway endpoints are limited to exactly **two services: Amazon S3 and DynamoDB**. Every other AWS service that supports private connectivity (Kinesis, Secrets Manager, ECR, CloudWatch Logs, SSM, etc.) requires an **Interface endpoint** instead. If a question asks about private access to any service other than S3/DynamoDB, the Gateway endpoint option is a distractor.
:::
...
:::warning Egress-only Internet gateway is for **IPv6 traffic**.
...
:::info Bastion instance vs NAT instance
...
:::info Static and dynamic routing
...
:::warning Auto detach public IP when attach an Elastic IP
...
:::info AWS Resource Access Manager
...
:::info Log from VPC flow log to show ACL is stateless
...
<div class="table-container"><div class="table-contents disable-scroll"><table id="w285aac11c11b5"><thead>
    ...
    <tr>
      ...
      <td>You can choose the IPv4 CIDR block for your VPC or you can allocate a <br/>CIDR block from Amazon VPC IP Address Manager (IPAM). For more information, see <a href="https://docs.aws.amazon.com/vpc/latest/ipam/what-is-it-ipam.html">What is IPAM?</a> in the<em>Amazon VPC IPAM User Guide</em>.</td>
      <td>You can bring your own IPv6 CIDR block to AWS for your VPC, choose an<br/>Amazon-provided IPv6 CIDR block, or you can allocate a CIDR block from Amazon VPC IP<br/>AAddress Manager (IPAM). For more information, see <a href="https://docs.aws.amazon.com/vpc/latest/ipam/what-is-it-ipam.html">What is IPAM?</a> in the<em>Amazon VPC IPAM User Guide</em>.</td>
    </tr>
    <tr>
