...
title: S3
sidebar_position: 1
description: Object storage service with strong consistency, lifecycle policies, storage classes, Glacier archiving, and lifecycle transitions for cost optimization.
keywords:
  - aws
  - amazon web services
  - storage
...
:::warning Use Standard-IA only when you need big thoughput
...
:::warning Standard IA vs One Zone IA
...
:::warning What happen when a S3 object has more than 1 million version?
...
:::warning How to delete a bucket which enabled Versioning and MFA Delete features
...
:::warning 2 Writes are made to a single non-versioned object
...
:::warning CloudFront can only use HTTP to connect to S3 as a website endpoint
If your Amazon S3 bucket is configured as a website endpoint, you can't configure CloudFront to use HTTPS to communicate with your origin because Amazon S3 **doesn't support HTTPS connections** in that configuration.
:::

:::danger Exam trap: REST endpoint vs website endpoint changes what access control is possible
- **REST API endpoint** (`bucket.s3.region.amazonaws.com`): supports **Origin Access Identity (OAI)** or **Origin Access Control (OAC)**, so the bucket can stay fully private while CloudFront serves its content, and both `GET`/`PUT` are supported.
- **Website endpoint** (`bucket.s3-website-region.amazonaws.com`): supports static website features like index/error documents and redirect rules, but the bucket **must be public** (no OAI/OAC support) and only works over HTTP between CloudFront and the origin.

If a requirement needs CloudFront to serve S3 content **while keeping the bucket private**, the origin must be the REST endpoint, not the website endpoint — even though the website endpoint looks like the obvious choice for a "static website" scenario.
:::

:::warning You must give the S3 bucket the same name as the record that you want to use to route traffic to the bucket
...
- Resource-based policies are policies that you can attach to your resources (buckets and objects)
  - [Bucket Policies](https://docs.aws.amazon.com/AmazonS3/latest/dev/example-bucket-policies.html): Use the AWS IAM policy syntax to manage access for a particular S3 bucket and the objects in it; (A bucket policy is a resource-based policy - [Unlike an identity-based policy, a resource-based policy specifies who (which principal) can access that resource](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_compare-resource-policies.html).)
...
<table><thead>
<tr><th class="table-header" colspan="100"><div class="title">ACL permissions</div></th></tr>
<tr>
<th>Permission</th>
...
:::warning No OAI for S3 static website
...
:::info AWS OpsHub - an UI replacement of CLI to manage of Snow Family Devices
