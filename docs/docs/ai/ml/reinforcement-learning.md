...
The learning loop below would be worth visualizing because RL becomes easier to understand once the interaction cycle and algorithm branches are seen as one system.

<!-- NOTEBOOKLM_DIAGRAM: concept=reinforcement-learning-loop-and-branches; type=image; goal=show the agent-environment loop together with the split between value-based, policy-based, and actor-critic methods; complexity=intermediate -->

The future image should make two things clear: feedback is delayed across time, and the major algorithm families differ in what they learn directly.
...
That is the structural reason RL feels harder to stabilize. The training data is generated by the evolving policy itself, so the learning target and the data distribution move together.
...
In practice, when the observed state does not actually contain enough information for the Markov assumption to be approximately true, RL performance usually degrades quickly. State design is therefore part of the modeling problem, not merely an implementation detail.
...
Here, $G_t$ is the return starting at time step $t$, $\gamma$ is the discount factor, $R_{t+k+1}$ is the reward received $k+1$ steps into the future, and $k$ indexes how far ahead the sum looks.
...
In these definitions, $\pi$ is the policy, $s$ is a state, $a$ is an action, and the expectation means the value is averaged over future trajectories generated by following policy $\pi$.

These are the core objects of RL. They tell me how good a state is and how good an action is inside a state.

Those definitions matter because most RL algorithms are different ways of estimating, improving, or exploiting those value signals under limited data and noisy feedback.
...
Here, $P(s', r \mid s, a)$ is the probability of moving to next state $s'$ and receiving reward $r$ after taking action $a$ in state $s$, and $a'$ denotes a possible next action under the policy.
...
It is also one of the reasons offline evaluation in RL is difficult. A policy that looks promising under the current exploration pattern may behave very differently once deployed under a changed action distribution.
...
In the TD and Q-learning updates, $\alpha$ is the learning rate, $S_t$ and $A_t$ are the current state and action, and $\max_a Q(S_{t+1}, a)$ is the estimated value of the best next action in the following state.
...
That replacement is powerful, but it comes at a cost. Function approximation introduces generalization across states, yet it also makes the target harder to stabilize because prediction errors can propagate through the bootstrapped update loop.
...
Those differences are not just academic taxonomy. They affect data efficiency, action-space compatibility, and how brittle training becomes under noisy reward signals.
...
That ordering matters. Many RL problems that look like algorithm failures turn out to be environment, reward, or state-definition failures instead.
