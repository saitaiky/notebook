---
title: PGP, GPG, SOPS
description: 'PGP, originally created by Phil Zimmermann in 1991, is a data encryption and decryption program that provides cryptographic privacy and authentication for data co.'
keywords:
  - linux
  - security
  - pgp gpg sops
...
:::info Comparison and Interoperability
...
> To understand what symmetric and asymmetric in cryptography, check my other post [encryption](/software-development/system-design/security/encryption/#symmetric-encryption)
...
### Symmetric way to decrypt

:::info Decrypt within 10mins
...
While GPG is great for general-purpose encryption and decryption, SOPS (Secrets OPerationS) adds several features that are specifically designed for managing secrets, especially in the context of cloud infrastructure and software development. Here are some reasons why you might choose to use SOPS over just GPG:

### Why Use SOPS?

While GPG is great for general-purpose encryption and decryption, SOPS (Secrets OPerationS) adds several features that are specifically designed for managing secrets, especially in the context of cloud infrastructure and software development. Here are some reasons why you might choose to use SOPS over just GPG:

- **Structured Data Handling**: SOPS understands JSON, YAML, and other structured formats. It allows you to encrypt only specific parts of a document, leaving other parts in plain text. This is useful for configuration files where you only want to encrypt sensitive data like passwords and API keys, but leave other configuration settings readable.
- **Key Management**: SOPS can use multiple encryption methods (PGP, KMS, Vault, etc.). It can encrypt different parts of a file with different keys, making it more flexible in complex environments.
- **Automatic Decryption**: SOPS can automatically decrypt files when they are opened by certain editors or tools, streamlining the workflow for developers who need to work with encrypted configuration files.
- **Integration with CI/CD Pipelines**: SOPS is designed to integrate well with CI/CD pipelines and tools, ensuring that secrets are managed securely throughout the development lifecycle.
- **Version Control Friendliness**: SOPS maintains the original file structure and supports line-by-line encryption, making it more suitable for storing secrets in version-controlled repositories.

### Example Use Case

Imagine you have a configuration file `config.yaml` that contains both non-sensitive settings and sensitive secrets:
...
Using SOPS, you can encrypt just the sensitive parts:

```yaml
database:
  host: db.example.com
  port: 5432
  user: ENC[AES256_GCM,data:encrypted_user,iv:iv_value,tag:tag_value]
  password: ENC[AES256_GCM,data:encrypted_password,iv:iv_value,tag:tag_value]
api_key: ENC[AES256_GCM,data:encrypted_api_key,iv:iv_value,tag:tag_value]
```

This way, anyone who needs to see or edit the non-sensitive parts can do so without decrypting the entire file, while the sensitive parts remain protected.

## Keyservers (Asymmetric)

:::info A List of Keyservers
...
:::danger Only use a keyserver that you trust
