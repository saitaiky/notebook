---
title: "SSH"
description: 'SSH (Secure Shell) agent is a program that assists with SSH key management. In the context of SSH authentication, an SSH agent saves your private keys and communi.'
...
## SSH in AWS operations

SSH is one access path, not the default answer for every cloud host. [EC2](/aws/compute/ec2/) covers instance key pairs and
host access, while [AWS Systems Manager](/aws/management/ssm/) can provide audited session access without exposing an SSH
port or distributing long-lived host keys. Protect private keys locally, scope the AWS principal that can start a session,
and prefer a centrally logged access path for production systems.
