---
title: "File system"
description: 'NFS (Network File System) is basically developed for sharing of files and folders between Linux/Unix systems by Sun Microsystems in 1980. It allows you to mount y.'
sidebar_position: 3
...
## Apply filesystem concepts on AWS

The local filesystem model helps distinguish AWS storage types. [EBS](/aws/storage/ebs/) provides block devices that an
instance formats and mounts, [EFS](/aws/storage/efs/) provides shared NFS-compatible file storage, and
[S3](/aws/storage/s3/) is object storage rather than a mounted POSIX filesystem. Use
[FSx](/aws/storage/fsx-hpc/) when a managed specialised filesystem better matches the workload. The storage choice should
follow access semantics, sharing, durability, and performance requirements rather than a familiar directory interface.
