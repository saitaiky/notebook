---
title: Linux
description: 'Linux is an open-source operating system kernel initially developed by Linus Torvalds in 1991. As the core component of many operating systems known as Linux dist.'
sidebar_label: Home
sidebar_position: 0
keywords:
  - linux
...
Linux is the runtime foundation beneath most server, container, and cloud workloads in this notebook. Use this page as the
map: learn the shell and filesystem first, then users and processes, then isolation, boot, packages, and security. The
existing kernel notes continue below the map.

## Learning path

| Stage | Read | Why it comes here |
| --- | --- | --- |
| Command line | [Bash introduction](/linux/bash-intro/) → [practical Bash](/linux/bash-practical/) | Establish the interface used by the remaining operational notes. |
| Files and identity | [Filesystems and NFS](/linux/file-system/) → [POSIX users](/linux/posix-user/) → [UID, GID, and special permission bits](/linux/uid-gid-stickbit/) | Understand how Linux names data and enforces local ownership. |
| Execution | [Processes](/linux/process/) → [namespaces and cgroups](/linux/namespace/) | Connect application execution to resource isolation and containers. |
| Machine lifecycle | [Boot sequence](/linux/loading-sequence/) → [CPU architecture](/linux/cpu-architecture/) → [package management](/linux/package-manager/) | Explain how the machine starts, executes binaries, and manages installed software. |
| Secure operation | [Linux security](/linux/security/ssh/) | Apply remote-access, encryption, and host-security controls. |

## Connect Linux to software design and AWS

| Linux mechanism | Software-design concept | AWS application |
| --- | --- | --- |
| Process lifecycle and isolation | [Stateful versus stateless design](/software-development/others/stateful-vs-stateless/) | [EC2](/aws/compute/ec2/), [ECS](/aws/compute/ecs-ecr/), and [EKS](/aws/development/eks/) |
| Users, groups, and permissions | [Authentication and identity](/software-development/authentication/) | [IAM](/aws/management/iam/) and [role assumption](/aws/security/assume-role/) |
| Filesystems and network mounts | [Storage concepts](/software-development/others/storage/) | [EBS](/aws/storage/ebs/), [EFS](/aws/storage/efs/), and [FSx](/aws/storage/fsx-hpc/) |
| Shell automation and packages | [Delivery and operations](/software-development/devops/) | [AWS CI/CD](/aws/development/cicd/) and [Systems Manager](/aws/management/ssm/) |
...
Return to the [Software Development map](/software-development/) for the architecture and delivery paths, or continue to
the [AWS overview](/aws/) to apply these operating-system concepts to managed compute, storage, identity, and operations.
